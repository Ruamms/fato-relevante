...
- [ ] **Mais documentos do FNET como fonte de leitura** — para além de relatório gerencial e fatos relevantes: Comunicados ao Mercado ("Outros Comunicados Não Considerados Fatos Relevantes" carregam notícia materialmente relevante com frequência), editais/atas de Assembleia (emissões, troca de taxa, liquidação do fundo) e Demonstrações Financeiras (parecer do auditor — ressalva de auditoria é candidata a red flag). Nota: os Informes Estruturados NÃO entram — são os mesmos dados que já baixamos da CVM em massa, e lá o processamento é determinístico
- [ ] **Histórico entre relatórios (IA + flags)** — quando houver leitura/flags de um período anterior (ex.: 06/2026) e chegar o relatório seguinte (09/2026), exibir a evolução como FATO comparado: "em 06/2026 este fundo tinha N alertas [quais]; desde então a cota variou X% e o VP Y%". Cuidado de redação: NUNCA como acerto/erro de "previsão" (o selo não prevê, constata) — é complementação de contexto, sem afirmar causalidade. Matéria-prima já existe: leituras/*.json versionadas no git (histórico completo por fundo) + leituras/_historico.txt (registro de cada rodada)
